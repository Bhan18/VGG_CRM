...
export async function listRecords(q: RecordQuery = {}) {
  ...
  // Map to camelCase + flatten employee
  const items = (data ?? []).map((r: Record<string, unknown>) => {
    const emp = r.attendance_employees as AttendanceEmployeeJoined;
    const { attendance_employees, ...record } = r as Record<string, unknown>;
    return {
      ...record,
      employee: emp,
    };
...
export async function getRecord(id: string) {
  ...
  const { attendance_employees, ...record } = data as Record<string, unknown>;
