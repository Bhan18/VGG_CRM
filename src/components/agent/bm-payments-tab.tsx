...
// Branch-manager Payments tab (staff app) — record payments against plots
// that still owe something, with up to 3 proof photos, and track own
// recordings through approval. Everything recorded here enters as
// `pending`; the ledger updates only after admin approval on the website.
//
// No money is ever shown in this tab. The BM types the amount into the form
// and never sees a figure back — not a price, not a paid total, not a
// balance, not their own recorded amount. The list of customers is already
// filtered server-side to those with a live outstanding balance, so the
// screen says who owes without saying how much.
    ...
    <section className="space-y-4 px-4 pb-6 pt-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="agent-card p-3">
          ...
          <div className="text-[10px] text-[var(--brand-ink)]/50">awaiting approval</div>
        </div>
        <div className="agent-card p-3">
          <div className="flex items-center gap-1.5 text-[11px] text-[var(--brand-ink)]/55">
          ...
          <div className="text-[10px] text-[var(--brand-ink)]/50">in the ledger</div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2" style={{ paddingBottom: "calc(1rem + env(safe-area-inset-bottom, 0px))" }}>
        {customersLoading ? (
          <div className="py-10 text-center text-xs text-[var(--brand-ink)]/40">Loading customers...</div>
        ) : filtered.length === 0 ? (
          <div className="py-10 text-center text-xs text-[var(--brand-ink)]/40">
            {customers.length === 0 ? "No customers with an outstanding balance." : "No customers match."}
          </div>
        ) : (
          filtered.map((c) => (
            <button
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-[11px] text-[var(--brand-ink)]/55">
                  {c.phone ? `${c.phone} · ` : ""}
                  {c.plots.length === 1
                    ? `Plot ${c.plots[0]!.plotNumber}`
                    : `${c.plots.length} plots`}
                </div>
              </div>
              <div className="shrink-0 text-right">
                ...
                <div className="text-[10px] text-[var(--brand-ink)]/45">{c.plots.length} plot{c.plots.length === 1 ? "" : "s"}</div>
              </div>
            </button>
          ))
        )}
...
function RecordForm({ customers, customersLoading, onDone }: { customers: BmCustomer[]; customersLoading: boolean; onDone: () => void }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3 rounded-xl border p-3" style={{ borderColor: "color-mix(in srgb, var(--brand-emerald) 15%, #e5e0d4)", background: "color-mix(in srgb, var(--brand-emerald) 3%, white)" }}>
          ...
          {customer ? (
            <span className="min-w-0">
              ...
              <span className="block truncate text-[11px] text-[var(--brand-ink)]/50">
                ...
                {customer.plots.length} plot{customer.plots.length === 1 ? "" : "s"} with dues
              </span>
            </span>
          ) : (
      ...
      {/* The selected customer's plots — always listed, so the BM can see
          what the payment is landing on even with a single plot. */}
      {customer && (
        <div className="flex flex-col gap-1.5">
          <span className="text-xs font-medium text-[var(--brand-ink)]/70">
            Plot{customer.plots.length === 1 ? "" : "s"} *
          </span>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[10px] text-[var(--brand-ink)]/50">
                    {[p.block, p.projectName].filter(Boolean).join(" · ") || "Payment due"}
                  </div>
                </button>
              );
            })}
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white">
      <header className="flex items-center gap-3 border-b px-4 py-3" style={{ borderColor: "color-mix(in srgb, var(--brand-emerald) 12%, #e5e0d4)" }}>
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-semibold">{item.customerName ?? "Payment"}</div>
          <div className="truncate text-[10px] text-[var(--brand-ink)]/50">
            {item.plotNumber ? `Plot ${item.plotNumber}` : ""}{item.plotBlock ? ` · ${item.plotBlock}` : ""}
          </div>
        </div>
